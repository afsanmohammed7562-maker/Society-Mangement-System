...
checkRole(['admin', 'treasurer', 'member']);
include 'config/database.php';

$page_title = 'Treasurer Messages';
...
$can_post = in_array($_SESSION['role'], ['admin', 'treasurer']);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $can_post) {
    ...
    if (empty($title) || empty($content)) {
        ...
        $stmt = $conn->prepare("INSERT INTO messages (type, title, content, created_by) VALUES ('treasurer', ?, ?, ?)");
        $stmt->bind_param("ssi", $title, $content, $_SESSION['user_id']);
        if ($stmt->execute()) {
            $success_msg = "Treasurer message posted successfully!";
        } else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['delete_id']) && in_array($_SESSION['role'], ['admin', 'treasurer'])) {
    $delete_id = intval($_POST['delete_id']);
    $stmt = $conn->prepare("DELETE FROM messages WHERE id = ? AND type = 'treasurer'");
...
<div class="main-content">
    ...
    <div class="card form-card mb-4">
        <div class="card-header"><i class="bi bi-currency-dollar me-2"></i>Post Treasurer Message</div>
        <div class="card-body">
            <form method="POST" action="">
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" name="content" rows="5" placeholder="Write your message here..." required></textarea>
                </div>
                <button type="submit" class="btn btn-primary"><i class="bi bi-send me-1"></i> Post Message</button>
            </form>
        </div>
    </div>
    <?php endif; ?>
    ...
    <div class="card table-card">
        <div class="card-header"><h5 class="mb-0"><i class="bi bi-currency-dollar me-2"></i>All Treasurer Messages</h5></div>
        <div class="card-body p-3">
            <?php
            $msgs = $conn->query("SELECT m.*, u.U_name as author FROM messages m LEFT JOIN userd u ON m.created_by = u.id WHERE m.type='treasurer' ORDER BY m.created_at DESC");
                ...
                <div class="list-group-item px-0">
                    <div class="d-flex w-100 justify-content-between">
                        ...
                        <div class="text-end">
                            ...
                            <?php if (in_array($_SESSION['role'], ['admin', 'treasurer'])): ?>
            ...
            <?php else: ?>
            <p class="text-muted text-center py-4 mb-0">No treasurer messages yet.</p>
            <?php endif; ?>
        </div>
    </div>
</div>
