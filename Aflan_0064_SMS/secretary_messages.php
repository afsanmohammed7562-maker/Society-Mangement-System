...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $can_post) {
    ...
    if (empty($title) || empty($content)) {
        ...
        $stmt = $conn->prepare("INSERT INTO messages (type, title, content, created_by) VALUES ('secretary', ?, ?, ?)");
        $stmt->bind_param("ssi", $title, $content, $_SESSION['user_id']);
        if ($stmt->execute()) {
            $success_msg = "Secretary message posted successfully!";
        } else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['delete_id']) && in_array($_SESSION['role'], ['admin', 'secretary'])) {
    $delete_id = intval($_POST['delete_id']);
    $stmt = $conn->prepare("DELETE FROM messages WHERE id = ? AND type = 'secretary'");
...
<div class="main-content">
    ...
    <div class="card form-card mb-4">
        <div class="card-header"><i class="bi bi-envelope me-2"></i>Post Secretary Message</div>
        <div class="card-body">
    ...
    <div class="card table-card">
        <div class="card-header"><h5 class="mb-0"><i class="bi bi-envelope me-2"></i>All Secretary Messages</h5></div>
        <div class="card-body p-3">
            <?php
            $msgs = $conn->query("SELECT m.*, u.U_name as author FROM messages m LEFT JOIN userd u ON m.created_by = u.id WHERE m.type='secretary' ORDER BY m.created_at DESC");
            ...
            <?php else: ?>
            <p class="text-muted text-center py-4 mb-0">No secretary messages yet.</p>
            <?php endif; ?>
        </div>
    </div>
</div>
